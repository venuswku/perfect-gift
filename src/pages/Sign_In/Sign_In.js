...
class Sign_In extends Component {
  ...
  /* Make sure text box value is the same as the one in this.state. */
  changeHandler = (e) =>{
    this.setState({[e.target.name]: e.target.value})
  }
  
  componentDidMount() {
    /* Navigate user to home page if their sign in info is correct. */
    axios.get('http://localhost:3010/v0/authenticate', this.state) // port of the server
    .then(res => {
  ...
  /* Sends all sign in data in this.state to the server,
